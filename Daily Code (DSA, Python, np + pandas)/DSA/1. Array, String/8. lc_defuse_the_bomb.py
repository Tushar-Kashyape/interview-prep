...
"""
I was thinking of same approach but a bit complex to get index.
Modulo is the key to get the index in circular array.
"""
    ...
    for i in range(n):
        ...
        if k > 0:
            for j in range(1, k + 1):
                decrypt_sum += code[(i + j) % n]

        else:
            for j in range(1, abs(k) + 1):
                decrypt_sum += code[(i - j) % n]

        ans.append(decrypt_sum)

    return ans



# code = list(map(int, input("Enter code: ").split()))
# k = int(input())
code = [2, 4, 9, 3]
k = -2
