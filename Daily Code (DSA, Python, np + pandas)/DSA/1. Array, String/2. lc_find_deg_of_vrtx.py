"""
You are given a 2D integer array matrix of size n x n representing the adjacency
...
"""
Tried to solve it by myself -> list(list(map(int, input().split()))) - Wrong
Chatgpt explained correct way -> take n as it is n*n matrix then add it to []

Interviewers expect to know this -
"""
n = int(input("Enter n: "))
matrix = [list(map(int, input("Enter row: ").split())) for _ in range(n)]

print(find_degrees(matrix))