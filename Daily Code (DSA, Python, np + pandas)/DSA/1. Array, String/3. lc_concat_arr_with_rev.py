...
Construct a new array ans of length 2 * n such that the first n elements are the
same as nums, and the next n elements are the elements of nums in reverse order.
...
1 <= nums[i] <= 100
"""
def concat_with_rev(nums: list[int]) -> list[int]:
    return nums + nums[::-1]

nums = list(map(int, input("Enter the nums: ").split()))
print(concat_with_rev(nums))