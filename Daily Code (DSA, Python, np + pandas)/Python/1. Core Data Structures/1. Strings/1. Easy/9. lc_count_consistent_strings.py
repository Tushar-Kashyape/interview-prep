...
def count_consistent_strs(allowed: str, words:list[str]) -> int:
    count = 0
    allowed_set = set(allowed)
    for word in words:
        word_set = set(word)
        if word_set == allowed_set or word_set.issubset(allowed_set):
            count += 1

    return count
"""

# Negation: check for absence then negate it to get presence.
def count_consistent_strs(allowed: str, words:list[str]) -> int:
    count = 0
    allowed_set = set(allowed)
    for word in words:
...
allowed = input("Enter string: ")
words = list(input("Enter words: ").split())
print(count_consistent_strs(allowed, words))